...
**Caption/título:** *Painel / Bom dia, Joana*
...
4 `StatCard` (alunos ativos, certificados emitidos, taxa de conclusão, cursos publicados), delta em chip `--success`, número em pt-BR ("+4,2%"). Grade 2fr/1fr: `Table` "Matrículas recentes" (avatar+e-mail, curso, progresso em barra pílula, status em chip) + coluna "Precisa da sua atenção" (redações, denúncias, certificados prontos — prazos em chip `--warning`) + "Cursos mais concluídos" em barras `--success`. Ações: Exportar CSV (tonal), Novo curso (primary).

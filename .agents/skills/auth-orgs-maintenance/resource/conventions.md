...
Unauthorized from either path = Laravel default 403. Feature tests assert `assertForbidden()`, never a redirect. Route `role:` middleware (`tenancy-maintenance` (`resource/conventions.md`)) is first defense; Policy is second, enforced whatever route reaches the controller.
...
only through an invitation link (see `invitations-maintenance` (`resource/architecture.md`)) or a

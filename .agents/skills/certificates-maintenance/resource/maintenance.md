...
HTTP process); `DatabaseMigrations` retired (per-method `migrate:fresh`)
 — see `laravel-dusk`/`testing-maintenance` (`resource/conventions.md`). They also seed `Certificate`
rows directly with `Certificate::create([...])` rather than
`CertificateFactory` where exact `validation_hash` must
match formula in `resource/conventions.md` byte-for-byte (factory
...
- **Duplicate-key `QueryException` on re-completion.**
  ...
  `resource/architecture.md` idempotency section. If this surface as
...
- **Certificate mail link lands on the wrong portal.** Queued
  ...
  — see `notifications-maintenance` (`resource/conventions.md`).
- **PDF/public page show wrong Organization (or throw) for Gestor viewing
  ...
  `null`/wrong Course. See `tenancy-maintenance` (`resource/architecture.md`) for general pattern.
...
string (don't double-prefix). See `resource/conventions.md` for the

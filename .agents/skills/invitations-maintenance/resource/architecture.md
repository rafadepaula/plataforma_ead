...
`auth-orgs-maintenance` (`resource/architecture.md`)).
...
`courses-maintenance` (`resource/architecture.md`) documents. So `EnrollmentController` authorizes
...
- `courses-maintenance` (`resource/architecture.md`) — `Course::students()`/`User::courses()`, pivot
...
- `tenancy-maintenance` (`resource/architecture.md`) — `OrgScope`, `RolesEnum`, host resolution, why
  student tenancy lives in `credentials`/`course_user`, not on the
  `users` row.
- `auth-orgs-maintenance` (`resource/architecture.md`) — `credentials` schema (including the

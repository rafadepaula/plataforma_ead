...
`resource/architecture.md`. Call sites only pick the *reason*: `::notFound()`
...
`auth-orgs-maintenance` (`resource/conventions.md`) for the `{user}` segment.
...
- `courses-maintenance` (`resource/conventions.md`) — pivot-only `course_user` upsert shape the
  manual-enroll panel follows.
- `auth-orgs-maintenance` (`resource/conventions.md`) — the `credentials` enum and per-org account

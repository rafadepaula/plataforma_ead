...
- The copy handlers bind on `DOMContentLoaded` via
  ...
  `resource/conventions.md` — the controller parameter MUST be named
...
- Inherits `DatabaseTruncation` from `Tests\DuskTestCase`, declares no
  ...
  `laravel-dusk`/`testing-maintenance` (`resource/architecture.md`)).
...
(see `courses-maintenance` (`resource/conventions.md`)). Course availability no longer gates
...
(`resource/architecture.md`, `resource/conventions.md`,
`resource/maintenance.md`) in same change, before task counts as done.
Also re-check:
...
- `courses-maintenance` (`resource/maintenance.md`) — analogous module this one mirrors (enrollment
  panel + pivot revocation semantics).
- `tenancy-maintenance` (`resource/maintenance.md`) — underlying `OrgScope` contract this module
  builds on.

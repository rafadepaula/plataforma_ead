...
(see `quizzes-maintenance` (`resource/conventions.md`)).
...
resolved value (see `resource/architecture.md`).
...
confirming against open question logged in `resource/maintenance.md`.
...
`courses-maintenance` (`resource/conventions.md`)), `signature` (plain text/textarea, stored verbatim as

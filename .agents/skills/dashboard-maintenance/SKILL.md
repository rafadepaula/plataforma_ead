...
description: >
  ...
  admin-only-vs-gestor-editable) still needing decision. Use when
  `OrgDashboardTest`, `MultiTenantCsvExportTest`, or `DashboardDuskTest`
  fail; KPI show wrong (cross-org) number; CSV export silently buffer
  instead of stream; or Dashboard sidebar link stay dead (`#`).
license: MIT
metadata:
  feature: dashboard
  role: maintenance
...
- `tests/Feature/OrgDashboardTest.php` — Admin with no impersonated Org
  ...
  that Org, Gestor see only own `org_id`.
...
- `tests/Browser/DashboardDuskTest.php` (Dusk E2E) — Admin dashboard
  ...
  export link `href`, settings edit screen persisting org override.

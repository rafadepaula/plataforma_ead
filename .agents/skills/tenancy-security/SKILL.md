...
| `aluno`            | One credential per Organization held | Sees/ accesses only the HOST Organization's courses (catalog and classroom are host-scoped; enrollment alone in a foreign-org course grants nothing on this portal). Access granted **only** through active course enrollment relations within the host org. |

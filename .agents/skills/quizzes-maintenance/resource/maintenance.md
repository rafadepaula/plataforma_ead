...
- `tests/Browser/QuizAuthoringDuskTest.php` — single
  lifecycle-chain test (per `testing-maintenance` (`resource/conventions.md`) journey-grouping
...
this skill and `resource/conventions.md` again.
...
for a timed Quiz (see `resource/conventions.md`), so reloading the page does
...
`QuizTaking.js`
**must** update all three quizzes skills (`resource/architecture.md`,
`resource/conventions.md`, `resource/maintenance.md`) in same change, before task
...
- `courses-maintenance` (`resource/maintenance.md`) — analogous reorder/Policy cascade-authorize
  pattern this module copies one level deeper.
- `learning-maintenance` (`resource/maintenance.md`) — `MarkLessonCompleteAction`,

...
`quiz_attempts_open_slot_unique` index (see `resource/architecture.md`).
...
`resource/architecture.md` — Lesson Form Wiring). The standalone
...
authoring (see `resource/maintenance.md` for why this superseded earlier
two-screen design).
...
`courses-maintenance` (`resource/conventions.md`)).

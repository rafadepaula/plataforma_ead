...
and `tenancy-maintenance` (`resource/architecture.md`). Tenant boundary implied purely by FK chain up
...
- `max_attempts` (nullable): when set, blocks new submission once student
  ...
  `in_progress` attempt (see edge case in `resource/conventions.md`). `null`
...
1. Verify student has active enrollment in Quiz's Course's Org (same
   ...
   uses — see `learning-maintenance` (`resource/architecture.md`)).
...
5. If no pending essay question (none exist, or `GradeEssayAnswerAction`
   ...
   unchanged; see `learning-maintenance` (`resource/architecture.md`)).
...
- `courses-maintenance` (`resource/architecture.md`) — Lesson this feature hangs off, plus
  cascade-inherited-tenancy pattern this module copies one level deeper.
- `learning-maintenance` (`resource/architecture.md`) — `MarkLessonCompleteAction`,
  `LessonMarkedAsCompleted`, `EnsureStudentIsEnrolled`, all reused
  unmodified by student-facing side of this feature.
- `tenancy-maintenance` (`resource/architecture.md`) — `OrgScope`, `RolesEnum`.

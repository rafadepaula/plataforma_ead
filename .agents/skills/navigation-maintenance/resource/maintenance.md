...
### Badge count wrong / leaks across orgs
- **Cause:** resolver not org-scoped. `QuizAttempt` scopes via `whereHas('quiz.lesson.module.course')`. `ForumReport` has NO `OrgScope` — MUST filter through `postable()` + `ForumTopicPolicy`/`ForumReplyPolicy` `view` gate (see `resource/architecture.md`).

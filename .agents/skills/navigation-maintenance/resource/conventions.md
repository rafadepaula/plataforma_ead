...
2. **Org-scope it** — see `resource/architecture.md`. Model has `OrgScope`: `whereHas` through that relation is enough. No `OrgScope` (e.g. `ForumReport`): filter via `postable()` + Policy `view` gate. Bare unscoped `->count()` = cross-tenant leak.

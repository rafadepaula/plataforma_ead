...
2. **Redacts** `password`/`remember_token` from both arrays before persisting — required even for cast/hidden attributes, since `getChanges()`/`getOriginal()` expose them regardless of `$hidden`/casts. Exact call: `resource/conventions.md`.

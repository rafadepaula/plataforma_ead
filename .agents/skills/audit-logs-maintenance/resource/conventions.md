...
Wrap every call site in `try`/`catch` that swallows (same shape as `SendEnrollmentConfirmedNotification`, see `notifications-maintenance` (`resource/conventions.md`)) so audit failure **never** breaks the primary flow:
...
Inside `AuditService::log()`, only the **DB** write is try/catch-wrapped. Monolog write must still run when DB write fails, and reverse — duplo-armazenamento contract in `resource/architecture.md`.
...
- `org_id` — Admin only. **Must** be stripped for non-Admin even when present in query string, like `ReportExportController`'s spoofed-`org_id` guard (`dashboard-maintenance` (`resource/conventions.md`)).
...
`export()` reuses `index()` filter logic — extract to private `applyFilters()` so the two cannot drift. Streams full filtered set, not current page: follow `CsvStreamExportService`'s `chunk()`/`lazy()` inside `streamDownload()` (see `dashboard-maintenance` (`resource/architecture.md`) CSV Streaming Contract). `->paginate()->items()` silently truncates export to one page.

# Testing Conventions (`resource/conventions.md`)
...
`actingAs` fixa o host automaticamente (admin → `ADMIN_HOST`; org-bound → host da credential dele). Para outro host, chame `onHost()` depois — helpers em `tests/TestCase.php`, detalhes em `resource/architecture.md`.

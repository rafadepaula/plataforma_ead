# Testing Maintenance (`resource/maintenance.md`)
...
Testes E2E deste repositório agrupam jornada inteira em um método (ver `resource/conventions.md`). Isso muda diagnóstico de falha:
...
- Em Feature tests o análogo é esquecer `onHost()`/`actingAsOrgUser()` — host errado = estado zero ou org alheia; helpers em `tests/TestCase.php` (ver `resource/architecture.md`).

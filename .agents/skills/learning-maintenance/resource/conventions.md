...
`index()`'s `with()` (see `resource/architecture.md`'s multi-org section for
...
view contract (see `resource/architecture.md`). Do not reintroduce the
...
- `lesson-row` reads `$lesson->glyph` and the `completed` prop; it must
  ...
  `pdf_path` — that path is media-blind (see `resource/architecture.md`).
...
HTML and breaks the row click target. See `bootstrap-maintenance` (`resource/conventions.md`) §3.

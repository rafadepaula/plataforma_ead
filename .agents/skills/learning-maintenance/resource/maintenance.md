...
- **Selector rename**: the pre-rebuild catalog used
  ...
  `<x-course.card>` family; see `resource/conventions.md`). If a Dusk test
...
- **Card CTA disabled state**: when `ctaHref` is `null` (see
  `resource/architecture.md`'s CTA-degrades-to-null rule), `course-continue-
...
(`resource/architecture.md`, `resource/conventions.md`, `resource/maintenance.md`)
...
- `courses-maintenance` (`resource/maintenance.md`) — analogous module this one read Course/Module/
...
- **A revoked certificate reads as "never issued"**: the certificate
  ...
  and branch on `isRevoked()` instead (see `resource/architecture.md`).

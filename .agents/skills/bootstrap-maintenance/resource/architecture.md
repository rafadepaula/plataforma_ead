...
> `resource/conventions.md` §4 para o mapeamento atualizado. Os ~35 partials de
...
> — ver `resource/maintenance.md` §3.1 sobre a exceção). Partial novo:
...
— ver `resource/conventions.md` §3 item 4 sobre quando um bloco novo vira
...
| 634 atributos `style="..."` | classes utilitárias + classes de componente | zero `style=` é regra de lint (`resource/conventions.md`) |
...
| erros de validação renderizados à mão | `.is-invalid` + `.invalid-feedback` | ver `resource/conventions.md` |
...
- **PDF é território separado.** `resources/views/certificates/pdf.blade.php` roda em `barryvdh/laravel-dompdf`, que **não** entende CSS do Bootstrap 5 (custom properties, `color-mix()`, flexbox moderno, grid). A view de PDF mantém CSS próprio em `<style>` — **única** exceção à regra de zero CSS ad-hoc. Ver `resource/maintenance.md`.

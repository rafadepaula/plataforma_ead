...
1. **`style="..."` em view, componente ou layout.** Zero exceção em `resources/views/`, salvo `certificates/pdf.blade.php` (dompdf, ver `resource/maintenance.md`) e valor genuinamente dinâmico em runtime (`style="width: {{ $pct }}%"`) — e mesmo esse prefere `.progress-bar` + `aria-valuenow`.
...
12. **Markup Bootstrap cru em tela** quando existe (ou deveria existir) um `<x-ui.*>` — ver mandato de componentização em `resource/architecture.md`.
...
> (ver `resource/architecture.md`). Pontos que mudam o que você escreve na Blade:
>

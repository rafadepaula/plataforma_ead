...
- **Solução:** conferir ordem dos 4 imports de `app.scss` (tokens → `bridge`
  → `bootstrap` → `components/index`, ver `resource/architecture.md`); nunca
...
- **Solução:** respeitar a ordem dos 4 imports de `app.scss`: tokens `_ds/`,
  `bridge`, `bootstrap`, `components/index`. Ver `resource/architecture.md`.
...
- **Sintoma:** elemento renderiza sem estilo algum (não é "canto reto",
  ...
  classe não é uma das "fantasmas" listadas em `resource/conventions.md` §3.5
  nem Tailwind.
...
- **Solução aplicada:** abaixo de `sm`, rótulo em texto vira ícone +
  ...
  de iniciais). Ver `topbar.blade.php` e `resource/architecture.md` para o
...
Regra completa: `testing-maintenance` (`resource/conventions.md`).
...
Não adicione outra classe de raio por cima. Ver `resource/conventions.md`

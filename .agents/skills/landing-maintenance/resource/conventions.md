...
- **Never put a new `dusk=` on a wrapper.** No per-card, per-band or
  ...
  snapshot (see `resource/maintenance.md`).
...
all** (see `certificates-maintenance` (`resource/architecture.md`)). It is public but **host-scoped**:
...
- No tenant blade mounts a `<x-help-button>` — the landing page
  deliberately has no contextual help mapping (see `help-maintenance` (`resource/conventions.md`)).
...
- Shell: `<x-layout.public :container="false" surface="white"
  ...
  is full-bleed with token-driven gutters (see `resource/architecture.md`).

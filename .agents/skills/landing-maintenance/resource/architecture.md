...
band. See `help-maintenance` (`resource/architecture.md`) for the standalone-document bucket and
`help-maintenance` (`resource/maintenance.md`) for why a missing button there is a wiring omission
with no automatic enforcement.

## Related

- `resource/conventions.md` — `dusk=` contract, copy rules, footer link.
- `resource/maintenance.md` — tests and failure modes.
- `tenancy-maintenance` (`resource/architecture.md`) — `OrgContext`, state zero, host resolution the
  controller leans on.
- `certificates-maintenance` (`resource/architecture.md`) — the `certificates.verify` public route
  the footer links to.

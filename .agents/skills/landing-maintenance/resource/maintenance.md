...
- `tests/Feature/LandingPageTest.php` — tenant brand + key copy of the
  ...
  contextual help button (shared territory with `help-maintenance` (`resource/maintenance.md`), which
...
HTTP process). See `laravel-dusk`/`testing-maintenance` (`resource/conventions.md`).
...
- **`harness:check-skills` reports an incomplete `landing` skill.**
  `scripts/check-skills.php` auto-discovers every `*-maintenance/` directory
  with a `resource/` folder and then demands `SKILL.md` plus
  `resource/architecture.md`, `resource/conventions.md` and
  `resource/maintenance.md` for each. All ship together, and any
...
## Skill Harness Requirement (This Module's Own)

`.agents/skills/` must always hold the complete `landing-maintenance/` skill
(SKILL.md + the three mandatory references) for this feature:
...
re-writes the affected references before the task finishes. Adding a tenant
blade, changing the fallback ladder, changing the `dusk=` contract or the
brand composer each belong in a specific reference — `resource/architecture.md`
(controller ladder, blades, brand, tokens), `resource/conventions.md`

...
to Organization it impersonate now (see `tenancy-maintenance` (`resource/architecture.md`) Impersonate Org
...
| `course_user` (pivot) | `user_id`, `course_id`, `status` (`active`\|`cancelled`\|`completed`), `progress_percentage`, `enrolled_at`, `completed_at`, `expires_at` nullable | Not org-scoped — enrollment cross orgs (see `tenancy-maintenance` (`resource/architecture.md`)). `expires_at` backs `Course::enrollmentDisplayStatusFor()`'s derived `expirado` chip on an `active` row past deadline — not a 4th pivot `status` value |
...
`Course` use `OrgScope` trait directly (see `tenancy-maintenance` (`resource/architecture.md`)): every query
...
enforcement point for that case. See `resource/conventions.md` for exact pattern
...
transactional submit via `SaveQuizForLessonAction` — see `quizzes-maintenance` (`resource/architecture.md`)
...
- `tenancy-maintenance` (`resource/architecture.md`) — `OrgScope`, `RolesEnum`, Impersonate Org.
- `learning-maintenance` (`resource/architecture.md`) (student-facing course consumption,
  `lesson_progress`, publication visibility) and `quizzes-maintenance` (`resource/architecture.md`)
  (quiz authoring) — both build on top of this feature's schema without
  modifying it.
- The "Meus Cursos" student catalog (see `learning-maintenance` (`resource/architecture.md`)) — added

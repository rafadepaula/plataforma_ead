...
(see `quizzes-maintenance` (`resource/architecture.md`) — Lesson Form Wiring). Switching back to `content`
...
parent Course), not `auth()->user()`'s own org. See `resource/conventions.md` for why
...
(`resource/architecture.md`, `resource/conventions.md`, `resource/maintenance.md`) in same
...
- `auth-orgs-maintenance` (`resource/maintenance.md`) — analogous module this one mirror (CSV import chunking
  vs this module's AJAX reorder).
- `tenancy-maintenance` (`resource/maintenance.md`) — underlying `OrgScope` contract this module build on.

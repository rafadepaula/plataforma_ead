...
- Means `ForumTopicController::store()` `ForumTopic::withoutEvents()`
  ...
  key is Admin-Impersonate-Org-only (see `tenancy-maintenance` (`resource/architecture.md`)).
...
- 404 where 403 expected almost always mean controller method used typed
  ...
  `withoutGlobalScope('org')` pattern (see `resource/conventions.md`).
...
- Cause: a controller lookup regressed to bare `withoutGlobalScopes()`,
  ...
  `ForumReplyController::resolveTopic()` — see `resource/conventions.md`,
...
badge share, **must**
update all three forum skills (`resource/architecture.md`, `resource/conventions.md`,
`resource/maintenance.md`) in same change, before task counted done. Also
re-check `tenancy-maintenance` (`resource/architecture.md`) cascade-inherited table list for
`forum_replies` and its pseudo-polymorphic-table note whenever 5th forum
table or new `postable_type` added.

## Related

- `quizzes-maintenance` (`resource/maintenance.md`) — analogous `parentCourse()`-cascade Policy
  test-matrix pattern this module Policy tests copy one level shallower.
- `certificates-maintenance` (`resource/maintenance.md`) — analogous "resolve pseudo-polymorphic

...
`tenancy-maintenance` (`resource/architecture.md`)) overwrite `org_id` with the resolved tenant context —
...
- **"Fixado" is a STATIC chip.** `<x-ui.chip :static="true"
  ...
  `<span class="badge bg-info-subtle">` here (`bootstrap-maintenance` (`resource/conventions.md`)
  no-raw-Bootstrap-markup guardrail).

## Related Skills

- `quizzes-maintenance` (`resource/conventions.md`) — `parentCourse()` cascade-authorize pattern this
  module Policies mirror.
- `courses-maintenance` (`resource/conventions.md`) — `[data-reorder-url]`-style contract this

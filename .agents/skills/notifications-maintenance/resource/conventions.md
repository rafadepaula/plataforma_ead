...
`resource/architecture.md` for why no `User` recipient exists for that
trigger.
...
see `resource/architecture.md`. `NotificationTriggersTest` asserts the
...
- `forum-maintenance` (`resource/conventions.md`) — `ForumPolling.js`/`HttpClient` JS module contract
  `NotificationBell.js` mirrors.
- `tenancy-maintenance` (`resource/security.md`) — general "never trust route-bound id without manual

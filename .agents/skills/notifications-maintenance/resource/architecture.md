...
(`role:gestor`/`role:aluno`, see `resource/conventions.md`). No
...
`resource/conventions.md` for snippet.
...
- `certificates-maintenance` (`resource/architecture.md`) — `IssueCertificateAction`
  eligibility/idempotency engine trigger 2 hooks into.
- `forum-maintenance` (`resource/architecture.md`) — topic/reply schema trigger 3 recipient resolution
  walks, and why it deliberately excludes "new report" from trigger list.
- `invitations-maintenance` (`resource/architecture.md`) — why the per-student unique invitation
  dispatches nothing on issuance (the link is handed to the Gestor in the
  UI, not e-mailed).
- `tenancy-maintenance` (`resource/architecture.md`) — why `ForumTopic` `OrgScope` requires

...
`certificates-maintenance` (`resource/maintenance.md`) for full eligibility/idempotency picture this
...
- `$reply->topic` resolves `null` inside Listener/queued job: means
  ...
  `forum-maintenance` (`resource/maintenance.md`) for analogous `OrgScope` gotcha on forum side.
...
(see `resource/conventions.md` for exact call shapes and the
`OrgUrl` rationale).
...
`resource/conventions.md`. Do not narrow caught type.
...
notifications references (`resource/architecture.md`,
`resource/conventions.md`, `resource/maintenance.md`) in same change,
before task counts done. `.agents/skills` is only real location for these
references in this repo. `.ai/skills` and `.goose/skills` do not mirror
...
- `certificates-maintenance` (`resource/maintenance.md`) — `wasRecentlyCreated`/idempotency gotcha
  trigger 2 dispatch site sits inside of.
- `forum-maintenance` (`resource/maintenance.md`) — `withoutGlobalScopes()`/`OrgScope` gotcha trigger 3
  recipient resolution shares.
- `invitations-maintenance` (`resource/maintenance.md`) — why invitation issuance is notification-free

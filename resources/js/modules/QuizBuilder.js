 ...
 *   `is_correct` checkbox independent (N >= 1 correct options rule). The
 *   `[data-correct-checkbox]` input's rendered `type` is also swapped to
 *   `radio` for `single_choice`/`true_false` and back to `checkbox` for
 *   `multiple_choice` — a purely visual affordance, since these inputs are
 *   never given a shared `name` for native browser mutual-exclusion;
 *   `enforceSingleCorrect()` above remains the actual exclusivity
 *   mechanism regardless of the rendered `type`. Every `[data-option-row]`
 *   also gets `.is-correct` toggled to mirror its checkbox's checked state
 *   (`syncCorrectHighlight()`), so a marked-correct row is visually
 *   highlighted independent of the radio/checkbox swap.
...
export class QuizBuilder {
        ...
        document.querySelectorAll('[data-question-form]').forEach((form) => {
            ...
            form.addEventListener('change', (event) => {
                ...
                if (checkbox) {
                    ...
                    this.syncCorrectHighlight(form);
                }
            });
        });
    }
    ...
    applyTypeBehavior(suffix) {
        ...
        list.querySelectorAll('[data-option-row]').forEach((row) => {
            ...
            if (checkbox) {
                ...
                // Purely visual: single_choice/true_false read as a radio,
                // multiple_choice as a checkbox. These inputs never share a
                // `name`, so this never gives native browser mutual-
                // exclusion — `enforceSingleCorrect()` remains the actual
                // exclusivity mechanism regardless of this `type` value.
    ...
    /**
     * Toggles `.is-correct` on every `[data-option-row]` in `form` to
     * mirror its `[data-correct-checkbox]`'s current checked state — kept
     * separate from `enforceSingleCorrect()` so it also runs standalone
     * (e.g. from `applyTypeBehavior()`/`applyRowDisabledState()`) to keep
     * server-rendered and freshly-cloned rows in sync without needing a
     * `change` event to have fired first.
     */
    syncCorrectHighlight(form) {
        form.querySelectorAll('[data-option-row]').forEach((row) => this.syncRowHighlight(row));
    }
    ...
    applyRowDisabledState(row, suffix) {
        ...
        if (textInput) textInput.disabled = isEssay;
        if (hiddenIdInput) hiddenIdInput.disabled = isEssay;
        // A freshly cloned template row is always unchecked, so this never
        // adds `.is-correct` by default — it only keeps the row's class in
        // sync should the template itself ever change that assumption.
        this.syncRowHighlight(row);
    }
